...
describe('SummaryCard breach props', () => {
  it('renders ring with color blue.6 when breached is false/undefined', () => {
    ...
    // The RingProgress renders an svg
    const svg = container.querySelector('svg');
    expect(svg).toBeTruthy();
    // No breach → no red color references should appear
    expect(hasRedColorReference(container)).toBe(false);
  });

  it('renders ring with color red.6 when breached=true', () => {
    const { container } = renderCard({
      label: 'Test',
      value: '72%',
      icon: <span />,
    ...
    const svg = container.querySelector('svg');
    expect(svg).toBeTruthy();
    // With breached=true, Mantine maps 'red.6' into the DOM somewhere
    // (stroke, CSS var, or class). Assert the reference exists.
