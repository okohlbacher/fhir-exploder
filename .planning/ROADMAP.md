...
- [x] **Phase 38.1: Fix `_sort=-date` Blaze incompatibility (INSERTED)** — Surfaced by Phase 38 HUMAN-UAT walk on 2026-04-28 as 3 critical-severity hits in `33-HUMAN-UAT.md` (Tests 1, 2, 6). Blaze 1.6.2 rejects `_sort=-date` with HTTP 400 `"Unknown search-param 'date' in sort clause."` on every per-patient resource search. Swap `_sort=-date` → `_sort=-_lastUpdated` (or remove sort entirely) in 4 source files: `src/components/patients/MiiModuleTab.tsx:81`, `src/components/patients/ClinicalTimeline.tsx:72`, `src/components/patients/PatientTimeline.tsx:122`, `src/components/patients/FhirResourcesView.tsx:132`. After landing, append `→ fixed in Phase 38.1, commit <sha>` to each failing row in `33-HUMAN-UAT.md` (rows keep `result: fail` per D-03 — historical record of what the v1.5 walk surfaced). **Fully automatable.** (completed 2026-04-28)
- [x] **Phase 38.2: Fix ValueQuantity render — show value followed by unit on Observation rows (INSERTED)** — Surfaced 2026-04-28 during the Phase 38.1 re-walk. Lab Observation rows on the patient detail Laborbefund tab render the unit only (e.g. `mg/dL`) instead of the full quantity (e.g. `98 mg/dL`). The bug was masked by the `_sort=-date` regression closed in Phase 38.1 — once the tab populated, the value-rendering defect became visible. Locate the cell renderer / summary extractor that reads `valueQuantity.unit` without `valueQuantity.value` (likely in `MiiModuleTab.tsx`'s row rendering, the shared Observation summary helper, or `getResourceStatusByType` / sibling extractor in `SearchResultsPage.tsx`). Fix: render `valueQuantity.value` followed by ` ` + `valueQuantity.unit`; handle the case where one is missing. Add a regression test asserting both `value` and `unit` appear in the rendered DOM. **Fully automatable.** (completed 2026-04-29)
...
### Phase 38.2: Fix ValueQuantity render — show value followed by unit on Observation rows (INSERTED)
**Goal**: Make Observation rows on the patient detail Laborbefund tab (and any sibling render site that consumes the same Observation summary helper) render `valueQuantity.value` followed by `valueQuantity.unit` instead of just the unit. Today the cell shows `mg/dL` where it should show `98 mg/dL`. The bug was masked by the `_sort=-date` regression closed in Phase 38.1; once the Laborbefund tab populated, the value-rendering defect became visible during the 38.1 re-walk on 2026-04-28.
**Depends on**: Phase 38.1 (Laborbefund tab needs to render data for the bug to be observable; 38.1 unblocks this).
**Requirements**: Closes a UAT-FU-06 follow-up gap. The original UAT-FU-06 fix populated the panel; this gap is the rendering of populated rows. No new REQ-IDs — implementation-level fix to the Observation summary extractor used by the MII Laborbefund tab.
**Success Criteria** (what must be TRUE):
  1. Lab Observation rows on the patient detail Laborbefund tab render `<value> <unit>` (e.g. `98 mg/dL`) when both `valueQuantity.value` and `valueQuantity.unit` are present.
  2. Rendering handles the missing-field cases gracefully (only `value` → render `value`; only `unit` → render empty / dash, NOT just unit; neither → empty cell).
  3. Existing test suite stays green; new regression test asserts that for an Observation fixture with `valueQuantity: { value: 98, unit: 'mg/dL' }`, the rendered DOM contains BOTH `98` and `mg/dL`.
  4. Re-walk of Phase 33 HUMAN-UAT Test 1 / Test 6 (Laborbefund populated) confirms values render correctly inline against live Blaze + Synthea (same fingerprint pinned in `38-SESSION.md`).
**Plans:** 1/1 plans complete

Plans:
- [x] 38.2-01-PLAN.md — RED-GREEN regression for Quantity dispatch in ResourcePropertyTable.tsx (4 valueQuantity input shapes) + reorder Quantity branch above Coding branch to fix UCUM-coded Quantity collapse + live-Blaze re-walk of Phase 33 HUMAN-UAT Tests 1, 6 with append-only D-03 evidence chain
**Effort**: < 0.5 day (single-site rendering fix + regression test + re-walk)
**Execution**: Mostly automatable — final re-walk is human-execution-bound (same Blaze + Synthea session as 38.1).
...
| 38.2. Fix ValueQuantity render on Observation rows | v1.5 | 1/1 | Complete    | 2026-04-29 |
