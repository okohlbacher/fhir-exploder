...
---
*Last updated: 2026-04-25 — v1.5 in progress. Phase 34 (14 MII Extension Modules + Palette + Bundled Profiles) complete: 14 extension modules added to `MII_MODULES` (Onkologie, Kardiologie, Intensivmedizin, Bildgebung, Pathologie, Mikrobiologie, Molekulargenetik, Seltene Erkrankungen, Symptom, Biobank, Studie, Dokument, MTB, PRO) with per-module `fhirResourceType` arrays for the 6 multi-profile modules, 3 `patientSearchParamOverrides` (biobank/pathologie Specimen→subject; studie ResearchStudy→enrollment), and `category: 'extension'` flagging; 7 custom `MantineColorsTuple`s landed in `src/theme.ts` (`oncology`, `imaging`, `genetics`, `pathology`, `bioanalysis`, `administration`, `patient-reported`) with WCAG AA contrast audit at `.planning/research/color-design-audit.md` (3 seed darkenings to clear 4.5:1 floor); 21 Tabler icons assigned via `src/utils/mii-icons.ts` `ICON_MAP` + `resolveMiiIcon()` rendered at 4 sites (Tabs pill 14px, Timeline dot 14px, Dashboard tile swatch 32px, Drawer 20px); `scripts/fetch-mii-profiles.mjs` wraps `fhir-package-loader@^2.2.4` (devDep) with `prepare` lifecycle + `|| true` offline-safety; 482 trimmed extension `StructureDefinition` JSONs committed under `src/quality/profiles/extensions/` with URL-keyed `EXTENSION_REGISTRY` mounted alongside base type-keyed `REGISTRY`; CC-BY-4.0 attribution via `ATTRIBUTION.md` + LICENSE appendix (MIT body preserved); empty-state UX via `useEmptyExtensionsCoordinator` Provider/reader/publisher trio + per-patient `localStorage.patients.hideEmptyExtensions.v1` Hide/Show toggle + 0.55 opacity dimming + em-dash copy ("— no {label} data for this patient") + base-7 D-21 exemption; D-24 test-count gate PASSED (998 / 0 failing). Shelved to backlog: Phase 999.2 (deuteranopia screenshots + TTI snapshot — paper predictions accepted in lieu) and Phase 999.3 (profile lazy-load — D-23 bundle-size +227.81 KB gz waived as feature-intrinsic). Next: Phase 35 — Phase-30 UAT Follow-ups + Per-Type Quality Matrix.*
