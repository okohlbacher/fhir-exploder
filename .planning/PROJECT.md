...
As of v1.2, the data quality surface is the most developed of the three entry points: it covers completeness, coding coverage, profile conformance, value set conformance, cardinality, temporal/age/duration plausibility, lab reference ranges, duplicate detection (patient matching + content hash), relational integrity (broken refs + orphans), configurable breach thresholds, trend history, and PDF export — all wired through a shared drill-down pattern that links every finding back to the underlying resource detail view.
...
**v1.2 shipped 2026-04-15** — 7 phases (14-20), 22 plans, +35,490 lines across 216 files. Builds cleanly (`tsc -b` + `npm run build` both exit 0). All 16 v1.2 requirements satisfied with traceability in `milestones/v1.2-REQUIREMENTS.md`. `v1.2-MILESTONE-AUDIT.md` gaps closed by Phase 20 (retrospective verifications authored for Phase 15 and Phase 18, DEBT-02 cast-widening fix). Current src size: ~28,400 LOC (TypeScript/TSX).
...
- Federated cohort queries (server-side CQL execution) — v1.3 is client-side definition only
- Cohort versioning / audit history — simple create/edit/delete; no change log
...
- **Persistence:** `localStorage` keyed by `quality.thresholds.v1` (thresholds), `quality.trends.v1` (snapshot history); v1.3 will add `quality.cohorts.v1`
- **Current codebase:** ~28,400 LOC TypeScript/TSX, `npm run build` + `tsc -b --noEmit` both clean
