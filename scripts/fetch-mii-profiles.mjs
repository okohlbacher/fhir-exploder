...
async function main() {
  ...
  // When all fetches fail (offline prepare-hook runs), importLines is
  // empty. Regenerating index.ts with zero imports would clobber the
  // committed static REGISTRY and break `getExtensionProfileForUrl()` for
  // every downstream consumer. ATTRIBUTION.md would also regenerate with
  // zero per-package sections (the attributionSections array is only
  // populated inside the loop, so it's also empty in this case). Bail out
  // and rely on the committed JSON/index.ts/ATTRIBUTION.md as the
  // authoritative offline fallback (CONTEXT D-12).
  // -----------------------------------------------------------------------
